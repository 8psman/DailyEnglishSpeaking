...
/**
 * Step 1: Get web content Step 2: Parse content to get list of chunk Step 3:
 * Download chunk
 */
...
public class JavaDemo {
	...
	public static void main(String args[]) throws IOException {
		...
		String webLink = "https://www.youtube.com/watch?v=3WoEQakN1i8";
		...
		if (info != null) {
			if (info.encodedStreams != null && info.encodedStreams.size() > 0) {
				...
				while (true) {
					...
					System.out
							.print("Invalid choice, choose again (0 to exit): ");
				}
				if (choice > 0) {
					EncodedStream stream = info.encodedStreams.get(choice - 1);
					boolean result = Youvider.downloadEncodedStream(stream,
							String.format("E:/%s.%s", info.videoTitle,
									stream.itag.format), null);
					if (result) {
